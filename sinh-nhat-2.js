...
function initBdayTheme() {
    ...
    if (!document.getElementById('tgdd-birthday-theme-styles')) {
        ...
        style.innerHTML = `
            ...
            /* Ngọn nến phát sáng - ĐÃ SỬA: Đưa vị trí nến khớp trên mặt bánh kem */
            .candle-group { 
                position: absolute; 
                top: 115px !important; 
    ...
    // =========================================================================
    // PHÂN KHU 2: PHÍM TẮT RESET BỘ NHỚ KIỂM TRA CHO CHỦ SỞ HỮU (CTRL+SHIFT+B)
    // =========================================================================
    window.addEventListener('keydown', (e) => {
        if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === 'b') {
            e.preventDefault();
            localStorage.removeItem('tgdd_bday_thu_seen_v1');
            localStorage.removeItem('tgdd_bday_last_closed_date');
            GM_deleteValue('tgdd_is_pending_render');
            GM_deleteValue('tgdd_pending_report_data');
            alert("🎉 Đã reset toàn bộ bộ nhớ Sinh Nhật của Hoài Thu! Đang tải lại trang để chạy thử nghiệm...");
            window.location.reload();
        }
    });

    // Ép kiểm tra Reset nếu trình duyệt có đính kèm chuỗi Hash: #test-bday
    if (window.location.hash === '#test-bday') {
        localStorage.removeItem('tgdd_bday_thu_seen_v1');
        localStorage.removeItem('tgdd_bday_last_closed_date');
        window.location.hash = ''; // Xóa hash để tránh lặp vòng lặp
    }

    // =========================================================================
    // PHÂN KHU 3: LOGIC ĐIỀU KHIỂN SỰ KIỆN TƯƠNG TÁC CHÚC MỪNG SINH NHẬT
    // =========================================================================
    const isOwner = (currentUserId === '42060') || (window.location.hash === '#test-bday');
    ...
    // =========================================================================
    // PHÂN KHU 4: ĐỒNG BỘ PHÍM BẤM CHẠY BÁO CÁO SỰ KIỆN (GIFT BOX TRANSFORMATION)
    // =========================================================================
    const observeAndTransformSpaceship = () => {
        const observer = new MutationObserver(() => {
            ...
            // 1. Tẩy sạch 100% các tia thiên thạch rơi dọc (Warp lines) để màn hình sinh nhật ấm áp, tinh khiết
            warpLines.forEach(line => line.remove());

            // 2. Chuyển đổi cưỡng chế SVG phi thuyền thành HTML Div chứa Emoji Hộp Quà 🎁 để vượt qua hạn chế hiển thị
            ...
            // 3. Chuyển đổi cưỡng chế SVG đuôi lửa thành Ngôi Sao Lấp Lánh ✨
    ...
    // =========================================================================
    // SỬA LỖI: Sử dụng "typeof UI" để kiểm tra bối cảnh nạp đồng bộ trước khi gán đè phương thức
    if (typeof UI !== 'undefined') {
        // Ghi đè phương thức hoàn thành báo cáo mặc định để bung mở pháo hoa dâu tây lấp lánh
        UI.showSuccessTick = (callback) => {
            ...
            const gift = shipWrapper.querySelector('.rocket-body');
            const star = shipWrapper.querySelector('.rocket-flame');
            ...
            // 3. Chờ 0.6s để người xem kịp ngắm nhìn hộp quà nổ pháo hoa rồi mới phóng vút thẳng lên trời
            setTimeout(() => {
                ...
                }, 600); // Khoảng thời gian hộp quà phóng bay khuất mốc trên màn hình
            }, 600);
        };
    }
